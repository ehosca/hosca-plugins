...
When showing it, substitute the plugin's real absolute path for `${CLAUDE_PLUGIN_ROOT}` — the
variable is not defined in the user's shell.
...
isn't stored yet, ask the user to store it once themselves (never in chat; same path
...
1. Run `${CLAUDE_PLUGIN_ROOT}/scripts/detect-sqlcmd.ps1` to locate sqlcmd; if missing, show its
   guidance and ask before installing go-sqlcmd (`winget install sqlcmd`).
2. Confirm server / database / auth with the user.
3. Execute `${CLAUDE_PLUGIN_ROOT}/skills/sql-audit/queries/audit.sql` via sqlcmd
   (pipe-delimited, headerless).
4. Cross-reference `${CLAUDE_PLUGIN_ROOT}/skills/sql-audit/references/celko-rules.md` and write `audit-report.md`
